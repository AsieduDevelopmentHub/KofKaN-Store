...
export function AccountOrdersPanel() {
  ...
  useEffect(() => {
    setHiddenIds(new Set(parseHiddenOrderIds(typeof window !== "undefined" ? window.localStorage.getItem(storageKey) : null)));
  }, [storageKey]);
  ...
  const visibleOrders = useMemo(() => {
    return orders.filter((o) => !hiddenIds.has(o.id)).sort((a, b) => b.created_at.localeCompare(a.created_at));
  }, [orders, hiddenIds]);
      ...
      {busy ? (
        ...
        <div className="mt-4 space-y-3">
          {visibleOrders.length === 0 ? (
            <p className="text-small text-kofkan-text-secondary">No orders to show.</p>
          ) : null}
              ...
              <article key={o.id} className="rounded-xl border border-kofkan-gray-soft bg-white p-4 dark:bg-zinc-900">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-semibold text-kofkan-text-primary">Order #{o.id}</p>
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${orderStatusPillClass(o.status)}`}>
                        {orderStatusLabel(o.status)}
                      </span>
                    </div>
                    <p className="mt-1 text-small text-kofkan-text-muted dark:text-zinc-500">
                    ...
                    <p className="mt-1 text-xs text-kofkan-text-muted">
                      {isDelivered ? "Reviews unlocked for this order." : "Reviews unlock when delivered."}
                    </p>
                  </div>
                ...
                {isDelivered ? (
                  <p className="mt-3 text-xs text-kofkan-text-secondary">
                    To leave a review, open this order and use the item(s) review button once it’s available.
                  </p>
                ) : null}
              </article>
            );
          ...
          {hiddenIds.size > 0 && showHidden ? (
            <>
              ...
              {hiddenOrders.length === 0 ? (
                <p className="text-small text-kofkan-text-secondary">Nothing hidden.</p>
              ) : null}
              {hiddenOrders.map((o) => (
                <article key={o.id} className="rounded-xl border border-kofkan-gray-soft bg-white p-4 dark:bg-zinc-900">
                  <div className="flex items-start justify-between gap-3">
